...
//--------------------GET BREEDER VIEW API-------------
  ...
  Future<ViewModel?> getViewModel({required BuildContext context}) async {
    method = "get";
    context = context;
    ResponseModel _responseModel = await request('get_breeder_view');
...
//--------------------UPDATE ACCOUNT SETTING API-------------
  ...
  Future<void> breederUpdateLicenceInfo(
    ...
    data = body;
    ResponseModel _responseModel = await request("update_license_info");
    if (_responseModel.statusCode >= 200 && _responseModel.statusCode < 300) {
      EasyLoading.showSuccess(CustomStyles.saveText);
    }
  }
...
//--------------------UPDATE PASSWORD API-------------
  ...
  changePassword(
    ...
    this.context = context;
    data = body;
    ResponseModel _responseModel = await request("change_password");
    if (_responseModel.statusCode >= 200 && _responseModel.statusCode < 300) {
