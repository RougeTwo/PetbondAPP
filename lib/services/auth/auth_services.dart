...
class AuthServices extends BaseService {
  ...
  login(
    ...
    redirectOn = redirect;
    data = body;
    ResponseModel _responseModel = await this.request("login");
  ...
  forgetPassword({required String email, required BuildContext context}) async {
    ...
    this.context = context;
    data = body;
    ResponseModel _responseModel = await this.request("forget_password");
    if (_responseModel.statusCode >= 200 && _responseModel.statusCode < 300) {
  ...
  Future<dynamic> resendVerificationEmail(
    ...
    method = "get";
    context = context;
    ResponseModel _responseModel = await this.request('verify_email');
    if (_responseModel.statusCode >= 200 && _responseModel.statusCode < 300) {
  ...
  vetSignUp(
    ...
    this.context = context;
    data = body;
    ResponseModel _responseModel = await this.request("vet_signup");
    if (_responseModel.statusCode >= 200 && _responseModel.statusCode < 300) {
      storage.setStore('token', _responseModel.body['access_token']);
      storage.setStore('auth_id', _responseModel.body['user']['id'].toString());
      storage.setStore('role', _responseModel.body['user']['role']);
  ...
  charitySignUp(
    ...
    this.context = context;
    data = body;
    ResponseModel _responseModel = await this.request("charity_signup");
    if (_responseModel.statusCode >= 200 && _responseModel.statusCode < 300) {
      storage.setStore('token', _responseModel.body['access_token']);
      storage.setStore('auth_id', _responseModel.body['user']['id'].toString());
      storage.setStore('role', _responseModel.body['user']['role']);
